install_distress() {
    ...
    install() {
        ...
        case "$OSARCH" in
          ...
          i386* | i686*)
            package=https://github.com/corpus-dev/distress_releases/releases/latest/download/distress_i686-unknown-linux-musl
          ;;
...
while [[ $direct_udp_mixed_flood_packets_per_conn -lt 1 || $direct_udp_mixed_flood_packets_per_conn -gt 100 ]]
    ...
    if [[ "$init_system" == "systemd" ]] && systemctl is-active --quiet distress.service; then
        sudo rm -rf /tmp/distress >/dev/null 2>&1
        sudo systemctl restart distress.service >/dev/null 2>&1
    elif [[ "$init_system" == "openrc" ]] && rc-service distress is-active >/dev/null 2>&1; then
        sudo rm -rf /tmp/distress >/dev/null 2>&1
        sudo rc-service distress restart >/dev/null 2>&1
    fi
...
distress_run() {
  ...
  if [[ "$init_system" == "systemd" ]]; then
    ...
    sudo rc-service x100 stop >/dev/null 2>&1
    sudo rc-service distress start >/dev/null 2>&1
  elif [[ "$init_system" == "runit" ]]; then
    sv stop mhddos >/dev/null 2>&1
    sv stop x100 >/dev/null 2>&1
    sv start distress >/dev/null 2>&1
  fi
}
...
distress_auto_enable() {
  ...
  if [[ "$init_system" == "systemd" ]]; then
    ...
    sudo rc-update del x100 >/dev/null 2>&1
...
distress_enabled() {
  ...
  if [[ "$init_system" == "systemd" ]]; then
    ...
    sudo rc-update is-active distress >/dev/null 2>&1 && return 0 || return 1
  else
    return 1
  fi
}
...
distress_get_status() {
  while true; do
    ...
    if [[ "$init_system" == "systemd" ]]; then
      ...
      sudo rc-service distress status
    elif [[ "$init_system" == "runit" ]]; then
      sv status distress
    fi
...
initiate_distress() {
   ...
   if [[ $? == 1 ]]; then
    ...
    ddos_tool_managment
      ...
      if sudo systemctl is-active distress >/dev/null 2>&1 || rc-service distress is-active >/dev/null 2>&1 || sv status distress >/dev/null 2>&1; then
