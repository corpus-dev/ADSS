x100_run() {
  ...
  if [[ "$init_system" == "systemd" ]]; then
    ...
    sudo rc-service x100 start >/dev/null 2>&1
  elif [[ "$init_system" == "runit" ]]; then
    sv stop distress >/dev/null 2>&1
    sv stop mhddos >/dev/null 2>&1
    sv start x100 >/dev/null 2>&1
  fi
}
...
x100_auto_enable() {
  ...
  if [[ "$init_system" == "systemd" ]]; then
    ...
    sudo rc-update add x100 >/dev/null 2>&1
  fi
  create_symlink
...
x100_enabled() {
  ...
  if [[ "$init_system" == "systemd" ]]; then
    ...
    sudo rc-update is-active x100 >/dev/null 2>&1 && return 0 || return 1
  else
    return 1
  fi
}
  ...
  if [ -r /etc/os-release ]; then
    ...
    case "$pkg_manager" in
      ...
      xbps-install)
        ...
        ;;
      emerge)
        sudo emerge -n app-containers/docker
...
initiate_x100() {
   ...
   local init_system
   init_system=$(get_init_system)
   if [[ "$init_system" == "systemd" ]] && systemctl is-active x100 >/dev/null 2>&1; then
     active_disactive="$(trans "Зупинка X100")"
   elif [[ "$init_system" == "openrc" ]] && rc-service x100 is-active >/dev/null 2>&1; then
     active_disactive="$(trans "Зупинка X100")"
   else
...
configure_x100() {
    ...
    local init_system
    init_system=$(get_init_system)
    if [[ "$init_system" == "systemd" ]] && systemctl is-active --quiet x100.service; then
        sudo systemctl restart x100.service >/dev/null 2>&1
    elif [[ "$init_system" == "openrc" ]] && rc-service x100 is-active >/dev/null 2>&1; then
        sudo rc-service x100 restart >/dev/null 2>&1
    fi
...
install_x100() {
    ...
    curl -L  --fail  --max-time 30  "https://github.com/corpus-dev/x100-for-docker/raw/main/docker/x100-for-docker.zip" -o "./x100-for-docker.zip"
