...
read -e -p "$(trans "Threads: ")" -i "$(get_mhddos_variable "threads")" threads
    ...
    if [[ "$init_system" == "systemd" ]] && systemctl is-active --quiet mhddos.service; then
        sudo rm -rf /tmp/_MEI* >/dev/null 2>&1
        sudo systemctl restart mhddos.service >/dev/null 2>&1
    elif [[ "$init_system" == "openrc" ]] && rc-service mhddos is-active >/dev/null 2>&1; then
        sudo rm -rf /tmp/_MEI* >/dev/null 2>&1
        sudo rc-service mhddos restart >/dev/null 2>&1
    fi
...
mhddos_auto_enable() {
  ...
  if [[ "$init_system" == "systemd" ]]; then
    ...
    sudo rc-update add mhddos >/dev/null 2>&1
  fi
  create_symlink
...
mhddos_enabled() {
  ...
  if [[ "$init_system" == "systemd" ]]; then
    ...
    sudo rc-update is-active mhddos >/dev/null 2>&1 && return 0 || return 1
  else
    return 1
  fi
}
...
initiate_mhddos() {
  ...
  if [[ $? == 1 ]]; then
    ...
    ddos_tool_managment
      ...
      if sudo systemctl is-active mhddos >/dev/null 2>&1 || rc-service mhddos is-active >/dev/null 2>&1 || sv status mhddos >/dev/null 2>&1; then
